...
import '../services/simplified_feed_service.dart';

class OptimizedDashboardWidget extends StatefulWidget {
  const OptimizedDashboardWidget({super.key});

  @override
  State<OptimizedDashboardWidget> createState() => _OptimizedDashboardWidgetState();
}

class _OptimizedDashboardWidgetState extends State<OptimizedDashboardWidget>
  ...
  String _searchQuery = '';
  String _selectedFilter = 'all'; // all, articles, medias, blogs
  ...
  void initState() {
    ...
    // Pré-charger les données en arrière-plan
    SimplifiedFeedService.preloadData();
  }
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      final feedFuture = SimplifiedFeedService.getUnifiedFeed(page: 1, limit: 30);
      final mediasFuture = SimplifiedFeedService.getAllMedias(page: 1, limit: 50);
      ...
      if (mounted) {
        if (feedResult['success'] == true) {
          ...
          // Ajouter les médias additionnels de la bibliothèque
          ...
          setState(() {
            ...
            _totalBlogs = _allItems.where((item) => item.type == 'blog').length;
          });
        } else {
          setState(() {
            _isLoading = false;
  ...
  Future<void> _loadMoreData() async {
    ...
    try {
      final result = await SimplifiedFeedService.getUnifiedFeed(
        page: _currentPage + 1, 
        limit: 20
      );
  ...
  Future<void> _performQuickSearch() async {
    try {
      final result = await SimplifiedFeedService.quickSearch(_searchQuery);
      ...
      // Erreur de recherche, garder le filtre local
    }
  }

  Future<void> _refreshData() async {
    // Vider le cache pour forcer le rechargement
    SimplifiedFeedService.clearCache();
    await _loadInitialData();
  }

  Future<void> _handleLike(FeedItem item) async {
    if (!await SimplifiedFeedService.isUserAuthenticated()) {
    ...
    try {
      final result = await SimplifiedFeedService.toggleLike(item.type, item.id);
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: _buildStatCard('Blogs', _totalBlogs, Icons.web_stories)),
            ],
          ),
        ],
      ),
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 8),
            _buildFilterChip('Blogs', 'blog', _totalBlogs),
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Chargement optimisé en cours...'),
          ],
        ),
      );
    }
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index < _filteredItems.length) {
            ...
            return _buildOptimizedFeedCard(item);
          } else {
  ...
  Widget _buildOptimizedFeedCard(FeedItem item) {
    return Card(
      ...
      child: InkWell(
        onTap: () {
          // Feature: Navigation to content detail page
        },
  ...
  Color _getTypeColor(String type) {
    switch (type) {
      ...
      case 'blog': return Colors.purple[100]!;
  ...
  Color _getTypeLabelColor(String type) {
    switch (type) {
      ...
      case 'blog': return Colors.purple[800]!;
